<x-app-layout title="Tambah UMKM">
    <div class="py-8 sm:py-10">
        <div class="container-page">
            <a href="{{ route('admin.umkms.index') }}" class="inline-flex items-center gap-1.5 text-sm font-medium text-[#8A7464] transition-colors duration-150 hover:text-[#5C4033] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C26A4A] focus-visible:ring-offset-2">
            ...
            <div class="mt-6">
                <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Tambah UMKM</h1>
                <p class="mt-1 text-sm text-slate-600">Buat UMKM atas nama pemilik sebagai layanan pendampingan. UMKM yang dibuat langsung berstatus Disetujui dan tampil di portal.</p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.umkms.store') }}" class="mt-6 space-y-6">
                @csrf
                    ...
                    <div class="border-b border-slate-100 px-5 py-4 sm:px-6 sm:py-5">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-0.5 text-xs text-slate-500">Tentukan pemilik dan kategori UMKM yang akan dibuat.</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            @if ($owners->isEmpty())
                                <p class="mt-2 rounded-xl bg-amber-50 px-4 py-3 text-sm leading-relaxed text-amber-800">
                                    Belum ada owner yang tersedia. Tambahkan owner terlebih dahulu di halaman <a href="{{ route('admin.users.create') }}" class="font-semibold underline">Kelola Pengguna</a>.
                                </p>
                            @else
                                <x-select id="owner_id" name="owner_id" class="mt-1 block w-full" required>
                                    <option value="">Pilih Owner</option>
                                    @foreach ($owners as $owner)
                                        <option value="{{ $owner->id }}" @selected(old('owner_id', request('owner')) == $owner->id)>
                                            {{ $owner->name }}{{ $owner->email ? ' ('.$owner->email.')' : '' }}
                                        </option>
                                    @endforeach
                                </x-select>
                                @if (request('owner') && $owners->contains('id', (int) request('owner')))
                                    <p class="mt-1 text-xs text-slate-500">Owner telah dipilih dari halaman Kelola Pengguna. Anda dapat menggantinya.</p>
                                @endif
                            @endif
                ...
                <div class="rounded-2xl bg-white shadow-sm">
                    ...
                    <div class="space-y-5 p-5 sm:p-6">
                        <div>
                            ...
                            <textarea id="address" name="address" rows="3" class="mt-1 block w-full rounded-xl border border-slate-300 bg-white py-2.5 px-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-[#C26A4A] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C26A4A] focus-visible:ring-offset-2">{{ old('address') }}</textarea>
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <x-text-input id="google_maps" name="google_maps" type="text" class="mt-1 block w-full" value="{{ old('google_maps') }}" placeholder="Contoh: https://maps.app.goo.gl/xxxxx" />
                            ...
                            <div>
                                ...
                                <x-text-input id="operational_hours" name="operational_hours" type="text" class="mt-1 block w-full" value="{{ old('operational_hours') }}" placeholder="Contoh: 08.00 - 17.00" />
                    ...
                    <div class="space-y-5 p-5 sm:p-6">
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <x-text-input id="phone" name="phone" type="text" class="mt-1 block w-full" value="{{ old('phone') }}" placeholder="Contoh: 0812-3456-7890" />
                                <x-input-error :messages="$errors->get('phone')" class="mt-2" />
                            </div>

                            <div>
                                <label for="email" class="block text-sm font-medium text-slate-700">Email</label>
                                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" value="{{ old('email') }}" placeholder="Contoh: usaha@email.com" />
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <x-text-input id="website" name="website" type="text" class="mt-1 block w-full" value="{{ old('website') }}" placeholder="Contoh: https://usaha.com" />
                                <x-input-error :messages="$errors->get('website')" class="mt-2" />
                            </div>

                            <div>
                                <label for="instagram" class="block text-sm font-medium text-slate-700">Instagram</label>
                                <x-text-input id="instagram" name="instagram" type="text" class="mt-1 block w-full" value="{{ old('instagram') }}" placeholder="Contoh: @nama.usaha" />
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div>
                                ...
                                <x-text-input id="facebook" name="facebook" type="text" class="mt-1 block w-full" value="{{ old('facebook') }}" placeholder="Contoh: nama.usaha" />
                                <x-input-error :messages="$errors->get('facebook')" class="mt-2" />
                            </div>

                            <div>
                                <label for="tiktok" class="block text-sm font-medium text-slate-700">TikTok</label>
                                <x-text-input id="tiktok" name="tiktok" type="text" class="mt-1 block w-full" value="{{ old('tiktok') }}" placeholder="Contoh: @nama.usaha" />
