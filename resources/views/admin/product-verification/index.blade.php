<x-app-layout title="Verifikasi Produk">
    <div class="py-12 sm:py-16">
        <div class="container-page">
            ...
            <h1 class="mt-6 text-3xl font-semibold tracking-tight text-slate-900">Verifikasi Produk</h1>
            <p class="mt-2 max-w-2xl leading-relaxed text-slate-600">Periksa pengajuan produk yang menunggu validasi.</p>
            ...
            @if ($requests->isNotEmpty())
                <p class="mt-6 text-sm text-slate-600">
                    Ada {{ $requests->count() }} pengajuan produk yang menunggu pemeriksaan Anda.
                </p>
            @endif
                        ...
                        @foreach ($requests as $request)
                            <li class="min-w-0 border-b border-slate-100 px-5 py-5 last:border-b-0 sm:px-6">
                                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                    <div class="min-w-0">
                                        <h3 class="break-words text-lg font-semibold leading-snug tracking-tight text-slate-900">{{ $request->verifiable->name }}</h3>

                                        <p class="mt-2 text-sm leading-relaxed text-slate-500">
                                            <span class="font-medium text-slate-400">UMKM:</span>
                                            <span class="break-words font-medium text-slate-700">{{ $request->verifiable->umkm?->name ?? '—' }}</span>
                                        </p>
                                        ...
                                        <dl class="mt-2 grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-3">
                                            <div class="min-w-0">
                                                ...
                                                <dd class="mt-0.5 break-words text-slate-700">{{ $request->user?->name ?? '—' }}</dd>
                                            </div>
                                            @if ($request->verifiable->category)
                                                <div class="min-w-0">
                                                    <dt class="text-xs font-medium text-slate-500">Kategori</dt>
                                                    <dd class="mt-0.5 break-words text-slate-700">{{ $request->verifiable->category->name }}</dd>
                                                </div>
                                            @endif
                                            <div class="min-w-0">
                                                <dt class="text-xs font-medium text-slate-500">Harga</dt>
                                                <dd class="mt-0.5 break-words font-medium text-slate-700">Rp {{ number_format((float) $request->verifiable->price, 0, ',', '.') }}</dd>
                                            </div>
                                            <div class="min-w-0">
                                                <dt class="text-xs font-medium text-slate-500">Diajukan</dt>
                                                <dd class="mt-0.5 break-words text-slate-700">{{ $request->created_at->format('d M Y') }}</dd>
                                            </div>
                                        </dl>
                                    </div>
                                    ...
                                    <div class="flex shrink-0 flex-col items-start gap-3 sm:items-end">
                                        ...
                                        <a href="{{ route('admin.products.verification.show', $request) }}" class="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 text-sm font-semibold text-white transition duration-300 hover:bg-emerald-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 sm:w-auto">
                                            Periksa
                                        </a>
                                    </div>
                                </div>
                ...
                @else
                    <div class="mt-4 rounded-2xl bg-white px-5 py-6 shadow-sm sm:px-6">
                        <p class="text-sm leading-relaxed text-slate-600">
                            Tidak ada pengajuan produk yang menunggu pemeriksaan. Anda sudah menangani seluruh pengajuan produk saat ini.
                        </p>
