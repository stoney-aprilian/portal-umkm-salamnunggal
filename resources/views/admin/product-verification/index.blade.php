<x-app-layout title="Verifikasi Produk">
    <div class="py-8 sm:py-10">
        <div class="container-page">
            ...
            <div class="mt-6 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Verifikasi Produk</h1>
                    <p class="mt-1 text-sm text-slate-600">Periksa dan validasi produk yang diajukan UMKM sebelum ditampilkan di portal.</p>
                </div>
                <p class="text-sm font-medium text-[#C26A4A]">{{ $pendingCount }} produk menunggu verifikasi</p>
            </div>
            ...
            <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <form method="GET" action="{{ route('admin.products.verification.index') }}" class="flex-1 sm:max-w-sm">
                    <label for="search" class="sr-only">Cari produk atau UMKM</label>
                    <div class="relative">
            ...
            <section class="mt-4" aria-label="Daftar pengajuan produk">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900">Pengajuan Produk</h2>
                    ...
                    <div class="overflow-hidden rounded-2xl bg-white shadow-sm">
                        ...
                        <ul class="divide-y divide-slate-100">
                            @foreach ($requests as $request)
                                @php
                                    $isRevision = $request->verifiable_type === \App\Models\ProductRevision::class;
                                    $product = $isRevision ? $request->verifiable->product : $request->verifiable;
                                    $photo = $product?->media->first();
                                    $umkmName = $product?->umkm?->name ?? '—';
                                @endphp
                                <li>
                                    <a href="{{ route('admin.products.verification.show', $request) }}" class="flex items-center gap-4 px-5 py-4 transition-colors duration-150 hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C26A4A] focus-visible:ring-inset sm:grid sm:grid-cols-12 sm:items-center sm:gap-4">
                                        <div class="hidden sm:col-span-4 sm:flex sm:items-center sm:gap-3">
                                            <div class="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
                                                @if ($photo)
                                                    <img src="{{ Storage::disk($photo->disk)->url($photo->path) }}" alt="Foto {{ $request->verifiable->name }}" class="h-full w-full object-cover">
                                                @else
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="mt-0.5 text-xs text-slate-500 sm:hidden">{{ $umkmName }} &middot; {{ $request->user?->name ?? '—' }}</p>
                                            </div>
                                        </div>
                                        <div class="hidden sm:col-span-2 sm:block">
                                            <p class="truncate text-sm text-slate-700">{{ $umkmName }}</p>
                                        </div>
                                        <div class="hidden sm:col-span-2 sm:block">
                                        ...
                                        <div class="hidden sm:col-span-1 sm:block">
                                            <p class="text-sm text-slate-700">Rp {{ number_format((float) $request->verifiable->price, 0, ',', '.') }}</p>
                                        </div>
                                        <div class="hidden sm:col-span-1 sm:block">
                                        ...
                                        <div class="sm:hidden">
                                            <div class="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <p class="mt-0.5 text-xs text-slate-500">{{ $umkmName }}</p>
                                                    <p class="mt-0.5 text-xs text-slate-500">{{ $request->user?->name ?? '—' }} &middot; {{ $request->verifiable->category?->name ?? '—' }} &middot; Rp {{ number_format((float) $request->verifiable->price, 0, ',', '.') }}</p>
                                                </div>
                ...
                @else
                    <div class="flex flex-col items-center justify-center rounded-2xl bg-white px-5 py-10 text-center shadow-sm sm:px-6">
                        ...
                        <p class="mt-1 text-sm text-slate-500">Tidak ada pengajuan produk yang menunggu pemeriksaan. Anda sudah menangani seluruh pengajuan produk saat ini.</p>
