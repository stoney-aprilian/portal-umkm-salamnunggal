            ...
            <div class="mt-6 overflow-hidden rounded-2xl bg-white shadow-[0_2px_12px_rgba(63,42,34,0.06)] ring-1 ring-[#ECE5D9]">
                <div class="p-6 sm:p-8">
                    ...
                    <div class="mt-8 flex flex-wrap items-center gap-3">
                        ...
                        @if (in_array($product->status, ['draft', 'needs_revision', 'rejected'], true))
                            <form method="POST" action="{{ route('owner.products.destroy', $product) }}" onsubmit="return confirm('Yakin ingin menghapus produk {{ $product->name }}? Foto dan pengajuan terkait juga akan dihapus permanen.');">
                                @csrf
                                @method('DELETE')
